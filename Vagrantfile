...
HOST_PORT = config_f["forward_ports"]["host"]           # port to forward to a host from a guest
GUEST_PORT = config_f["forward_ports"]["guest"]         # port to be forwarded from the guest
...
MAKE_EXEC = config_f["make_exec"]                     # files to chmod +x
...
Vagrant.configure(2) do |config|
    ...
    # appending commands to make files executable
    MAKE_EXEC.each do |script|
        env_mod << "echo 'sudo chmod +x #{script}' >> #{ENV_MOD_FILE}\n"
    end
